...
const CareerOptions = ({ openings }) => {
  ...
  if (!isJobActive(openings?.endDate)) return null;
    ...
    >
      <div className="flex items-center justify-between bg-accent px-5 py-3">
        <div className="flex flex-col text-left rtl:text-right">
          <Heading variant="small" className="text-blue">
            {openings?.title?.[currentLang] || ""}
          </Heading>

          {openings?.department?.name?.[currentLang] && (
        ...
        {!isHidden ? (
          <LuPlus className="text-blue opacity-70" strokeWidth={2.4} size={25} />
        ) : (
          <LuMinus className="text-blue opacity-70" strokeWidth={2.4} size={25} />
        )}
      </div>
